...
- Telegram alerts (if enabled) are best-effort and non-blocking; failures/timeouts never affect order processing. `ERROR` and `event=exit` produce Telegram messages.
